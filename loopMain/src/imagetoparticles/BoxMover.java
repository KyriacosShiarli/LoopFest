...
public class BoxMover{
	...
	// CONSTRUCTOR - - - - -  - -  -  - -  -  - -  - - - 
	public BoxMover(PApplet p, FWorld tempWorld, int tempParticleSize,int numberOfParticles) {
		...
		particles = new FBox[numberOfParticles];
		world = tempWorld;
			
		for (int i  = 0; i < numberOfParticles;i++){
			
			particles[i]= new FBox(particleSize,particleSize);
			particles[i].setPosition(100+i*particleSize/10, 100);
			particles[i].setSensor(false);
			particles[i].setDamping((float)15); // Default damping in 15 TODO -> function that changes all damping
	...
	public void	changeSize(int index,int width,int height)

	{
		if (height>70){return;}
		else{
			particles[index].setHeight(height);
			particles[index].setWidth(height);
		}
	}	
	...
	public void swap_cw(){
		if (numOfDrawable == 0){return;}
			...
			FBox temp = particles[numOfDrawable-1];
    ...
    public void changeColour(int colour){
    	for(int i = 0;i<numOfDrawable;i++){
    		if (colour==0){
    			...
    			particles[i].setStrokeWeight(2);
    			particles[i].setFill(200,colour,0,50);
    ...
    public void sizeOnPossition(){
    	...
    	for (int i = 0 ; i<particles.length;i++){
    		...
    		if (distance<20)
    			...
    			changeSize(i,(int)distance/10,(int)distance/10);
